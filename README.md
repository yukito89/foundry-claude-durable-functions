...
3. **デプロイ**
   ...
   - デプロイ完了後、Static Web AppのURLをメモ（例: `https://<your-static-app>.azurestaticapps.net`）

4. **Function AppのCORS設定**
   - Azureポータルで作成したFunction Appを開く
   - 「設定」→「CORS」を開く
   - 「許可されたオリジン」に以下を追加:
     - 前の手順でメモしたStatic Web AppのURL
     - ローカル開発用: `http://127.0.0.1:5500`（Live Server使用時）
   - 「保存」をクリック
...
5. **IPアドレス制限（オプション）**
   ...
   - 以下を設定:
     ...
     - **IPアドレスブロック**: 社内のIPアドレス範囲（例: `203.0.113.0/32`）
       - 現在のIPアドレスは[こちら](https://www.cman.jp/network/support/go_access.cgi)で確認できます
     - **アクション**: 許可
