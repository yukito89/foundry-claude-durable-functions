...
提供された設計書とテスト観点をもとに、実務でそのまま実行可能なテスト仕様書を作成してください。
...
1. 【前提条件】
   - ログイン状態: ユーザー名、権限、組織などを具体的に記載
   - 事前データ: DB登録済みデータを「テーブル名: 項目名=値」形式で記載
   - データがない場合は「なし」
...
【記述例】
良い例:
テストケース: 【前提】ログイン状態:一般ユーザー、事前データ:なし 【操作】1. ユーザーID="user001"、パスワード="pass123"を入力 2. 「ログイン」ボタンをクリック 【確認】トップ画面への遷移
期待結果: トップ画面が表示され、ヘッダに「user001」が表示されること
